...
3. **必须使用 bedrock-agentcore SDK** — 容器需使用 `BedrockAgentCoreApp` 模式，不能用纯 Flask
4. **Session ID 最小 33 字符** — AgentCore 要求
5. **filesystemConfigurations 需 raw API** — 当前 boto3 版本不支持 `efsAccessPoint`，需签名 PUT 请求

## 与腾讯云 CFS 方案对比

| 维度 | 腾讯云 CFS | AWS AgentCore + EFS |
|------|-----------|---------------------|
| 共享存储 | CFS | EFS (单 Access Point) |
| 沙箱引擎 | AGS | AgentCore Runtime |
| 隔离机制 | 控制面动态挂载子目录 | bind mount + 路径守卫 |
| Session 隔离 | 沙箱实例级 | microVM 物理隔离 |
| 业务 Pod 视图 | CFS 根目录 | EFS 根目录 |
| 沙箱视图 | 仅本租户子目录 | /workspace (bind mount) |
| 租户上限 | 无硬限制 | 无硬限制（不依赖 AP 数量） |
| 双向流转 | NFS 即时可见 | NFS close-to-open 一致性 |
